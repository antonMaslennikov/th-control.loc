...
BEGIN
    ...
    CREATE TABLE clients_pbn_sites_and_articles_new as
        ...
        FROM pbn_articles pa
            ...
            LEFT JOIN links_check_donor_acceptor lcda ON lau.id = lcda.id_url_from_donor AND lcda.date_check = CURRENT_DATE LEFT JOIN links_all_urls lau2 ON lcda.id_url_to_acceptor = lau2.id
    ...
    CREATE TABLE plan_fact as
        SELECT cp.pbn_owner, cp.site_url AS count_site_url, uc.acceptor_domain, uc.count_url_to_acceptor, pp.pbn_sites, pp.links, pp.deadline
        FROM
            (SELECT pbn_owner, COUNT(DISTINCT site_url) AS site_url FROM clients_pbn_sites_and_articles_new GROUP BY pbn_owner) cp
