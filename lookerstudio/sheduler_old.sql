...
BEGIN
    ...
    CREATE TABLE urls_check_new as
        SELECT lcda.date_check, lad.domain_name AS donor_domain, lau.url AS url_from_donor, laa.anchor_value, lau2.url AS url_to_acceptor, lad2.domain_name AS acceptor_domain,
            COALESCE( CASE WHEN c.client_name = 'old' AND lad2.domain_name LIKE 'fonbet%' THEN 'fonbet.by/fonbet.kz' WHEN c.client_name = 'old' THEN lad2.domain_name ELSE c.client_name END, lad2.domain_name) AS pbn_owner
        ...
        WHERE lcda.date_check = CURRENT_DATE();
    ...
    CREATE TABLE plan_fact as
        SELECT
            ...
            uc.acceptor_domain,
            COUNT(uc.url_to_acceptor) AS count_url_to_acceptor,
            pp.pbn_sites,
            pp.links,
            pp.deadline
        FROM
            (
                ...
                JOIN urls_check_new uc ON cp.pbn_owner = uc.pbn_owner AND uc.date_check = CURRENT_DATE()
                JOIN pbn_plans pp ON uc.acceptor_domain = pp.money_site
        GROUP BY cp.pbn_owner, uc.acceptor_domain, pp.pbn_sites, pp.links, pp.deadline;
